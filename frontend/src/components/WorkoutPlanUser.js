    ...
    <Modal
      ...
      slotProps={{
        backdrop: {
          timeout: 300,
          sx: {
            backgroundColor: "rgba(20,20,30,0.85)", // semi-transparent dark
...
function WorkoutPlanUser() {
  ...
  const [feedbackStats, setFeedbackStats] = useState({}); // { [planId]: { count, ratingSum, feedbacks } }
  const [modalOpen, setModalOpen] = useState(false);
  const [modalFeedbacks, setModalFeedbacks] = useState([]);
  const [modalPlanName, setModalPlanName] = useState("");
  const [showAddFeedback, setShowAddFeedback] = useState({}); // { [planId]: true/false }
  ...
  // Fetch plans and feedback stats
  useEffect(() => {
        ...
        await Promise.all(
          res.data.map(async (plan) => {
            const fbRes = await axios.get(
              `http://localhost:5000/api/workout-feedback/plan/${plan._id}`
            );
  ...
  const submitFeedback = async (planId) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/workout-feedback", {
        planId,
      ...
      const fbRes = await axios.get(
        `http://localhost:5000/api/workout-feedback/plan/${planId}`
      );
                ...
                <TableBody>
                  {plans.map((plan) => (
                    <TableRow key={plan._id} hover>
                      ...
                      <TableCell>{plan.height} cm</TableCell>
                      <TableCell>{plan.weight} kg</TableCell>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
                          <Tooltip title="View all feedbacks">
                            <Button
                              ...
                              variant="outlined"
                              color="primary"
                              onClick={() => handleOpenModal(plan._id, plan.workoutType)}
                            >
                              View All Feedbacks
                            </Button>
                          </Tooltip>
                        ...
                        {showAddFeedback[plan._id] && (
                          <Box sx={{ mt: 1, mb: 1, p: 1.5, bgcolor: "rgba(245,248,255,0.90)", borderRadius: 2 }}>
                            ...
                            <TextareaAutosize
                              ...
                              onChange={(e) =>
                                handleFeedbackChange(plan._id, "comment", e.target.value)
                              }
                              disabled={loading.feedback[plan._id]}
                            />
                            <Button
...
export default WorkoutPlanUser;
